...
(SELECT datepart(day, LocalTime) AS 'Day', ISNULL(COUNT([CardCode]), 0) AS 'Accessed Vehicles'
  ...
  WHERE [CardBits] = 34 AND [LocalTime] BETWEEN '2023-06-07 00:01:00.000' AND '2023-06-07 23:59:00.000' AND [DoorID] = 1
  GROUP BY datepart(day, LocalTime)),

pedestrian_time AS
(SELECT datepart(day, LocalTime) AS 'Day', ISNULL(COUNT([CardCode]), 0) AS 'Accessed Pedestrians'
  FROM [ASLog].[dbo].[AccessLog]
  WHERE [CardBits] = 25 AND [LocalTime] BETWEEN '2023-06-07 00:01:00.000' AND '2023-06-07 23:59:00.000' AND [DoorID] = 0
  GROUP BY datepart(day, LocalTime))
SELECT COALESCE(vehicle_time.Day, pedestrian_time.Day) AS 'Day', COALESCE(vehicle_time.[Accessed Vehicles], 0) AS 'Accessed Vehicles', COALESCE(pedestrian_time.[Accessed Pedestrians], 0) AS 'Accessed Pedestrians'
FROM vehicle_time
FULL JOIN pedestrian_time ON vehicle_time.Day = pedestrian_time.Day;



 ------------------------------ACCESS TIME PLOT PEAK-----------------------------------------------------
;WITH vehicle AS
(SELECT datepart(hour, LocalTime) AS 'Time', COUNT([CardCode]) AS 'Vehicle'
  FROM [ASLog].[dbo].[AccessLog]
  WHERE [CardBits] = 34 AND [LocalTime] BETWEEN '2023-06-07 00:01:00.000' AND '2023-06-07 23:59:00.000'
  GROUP BY datepart(hour, LocalTime)),
pedestrian AS
(
  SELECT datepart(hour, LocalTime) AS 'Time', COUNT([CardCode]) AS 'Pedestrian'
  FROM [ASLog].[dbo].[AccessLog]
  WHERE [CardBits] = 25 AND [LocalTime] BETWEEN '2023-06-07 00:01:00.000' AND '2023-06-07 23:59:00.000'
  GROUP BY datepart(hour, LocalTime))
