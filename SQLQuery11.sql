...
/****** Script for Select EVENTLOGS between march 2023 command from SSMS  ******/
SELECT
      DISTINCT[Name]
      ,[Gender]
      ,[EmployeeID]
...
SELECT COUNT(DISTINCT[CardNo]) AS 'Total V-Tag Card Codes Programmed into the System'
  FROM [ASConfig].[dbo].[Card]
  WHERE [CodeType]= 34

/****** Script for SelectTopNRows command from SSMS  ******/
SELECT COUNT([Name]) AS 'Total Registered Users in System'
  FROM [ASConfig].[dbo].[Cardholder]
...
SELECT COUNT(DISTINCT[CardNo]) AS 'TOTAL V-TAG CARDS TO RESIDENT-CARS (TOTAL CARS REGISTERED)'
FROM [ASConfig].[dbo].[Card]tb_1
JOIN [ASConfig].[dbo].[Cardholder]tb_2 ON tb_1.CardholderID=tb_2.CardholderID
WHERE [CodeType]=34
...
SELECT COUNT(DISTINCT[CardNo]) AS 'TOTAL Vehicles with Programmed Tags Captured for a Day 2023-03-22'
FROM [ASConfig].[dbo].[Card]tb_1
JOIN [ASLog].[dbo].[EventLog]tb_2 ON tb_1.CardholderID=tb_2.CardholderID
WHERE [LocalTime] BETWEEN '2023-03-22 00:01:00.000' AND '2023-03-22 23:59:00.000' AND [CodeType]= 34


 SELECT COUNT(DISTINCT[CardNo]) AS 'Total Captured Vehicles-Tags by Lane-02 and Lane-03 Tag-Readers from March 2023'
FROM [ASConfig].[dbo].[Card]tb_1
JOIN [ASLog].[dbo].[EventLog]tb_2 ON tb_1.CardholderID=tb_2.CardholderID
WHERE [LocalTime] >'2023-03-01 00:00:00.000' AND [CodeType]=34


SELECT COUNT(DISTINCT[CardNo]) AS 'Total Captured Vehicles-Tags by Lane-02 Tag-Reader from March 2023'
FROM [ASConfig].[dbo].[Card]tb_1
JOIN [ASLog].[dbo].[EventLog]tb_2 ON tb_1.CardholderID=tb_2.CardholderID
WHERE [CodeType]=34 AND [DoorID]= 1


SELECT COUNT(DISTINCT[CardNo]) AS 'Total Captured Resident Vehicles with Tags by Lane-03 Tag-Reader from March 20223'
FROM [ASConfig].[dbo].[Card]tb_1
JOIN [ASLog].[dbo].[EventLog]tb_2 ON tb_1.CardholderID=tb_2.CardholderID
WHERE [LocalTime] >'2023-03-01 00:00:00.000' AND [CodeType]=34 AND [DoorID]= 2

SELECT COUNT(DISTINCT[CardNo]) AS 'TOTAL NONE-CAPTURED PROGRAMMED V-CARDS BY THE TAG-READER'
