...
SELECT [RegisteredEstateResidents]
  FROM [master].[dbo].[RegisteredResidents];
...
WHERE [CodeType] = 34
-- Display the result from the table
SELECT TOP (1000) [RegisteredVehiclesCount]
  FROM [master].[dbo].[RegisteredVehicles]
...
-- Display the result from the table
SELECT TOP (1000) [CapturedRegisteredResidentVehicles]
  FROM [master].[dbo].[CapturedVehicles]
...
WHERE [CodeType] = 34 AND [Name] IS NOT NULL AND CardNo NOT IN (
    ...
    WHERE [LocalTime] > '2023-02-01 00:00:00.000' AND [CodeType] = 34);
-- Display the result from the table
SELECT TOP (1000) [RegisteredVehiclesNotCaptured]
  FROM [master].[dbo].[TotalRegisteredVehiclesNotCaptured]
...
-- Display the result from the table
SELECT TOP (1000) [AllCapturedTags]
  FROM [master].[dbo].[CapturedTags]
...
SELECT TOP (1000) [Day]
      ,[Accessed Vehicles]
      ,[Accessed Pedestrians]
  FROM [master].[dbo].[YourTableName]
